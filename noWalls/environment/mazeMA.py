...
class MazeMA:
    ...
    def __init__(self, maze, start_cells=(0, 0), exit_cell=None, num_agents=1):
        ...
        start_cells = [random.choice(self.cells) for i in range(num_agents)]
        exit_cell = (self.ncols - 1, self.nrows - 1) if exit_cell is None else exit_cell

        self.__exit_cell = exit_cell
        self.__previous_cells = self.current_cells = start_cells
        ...
        for agent_idx in range(num_agents):
            self.reset(start_cells[agent_idx], agent_idx=agent_idx)
    ...
    def reset(self, start_cell=(0, 0), agent_idx=0):
        ...
        self.__previous_cells[agent_idx] = self.current_cells[agent_idx] = start_cell
        ...
        # render the maze
            ...
            self.ax1.grid(True)
            self.ax1.plot(*self.current_cells, "rs", markersize=25)  # start is a big red square
        ...
        return self.__observe(agent_idx)

    def __draw(self):
        """ Draw a line from the agents previous to its current cell. """
        self.ax1.plot(*zip(*[self.__previous_cells, self.current_cells]), "bo-")  # previous cells are blue dots
        self.ax1.plot(*self.current_cells, "ro")  # current cell is a red dot
        # plt.pause(0.001)  # replaced by the two lines below
        self.ax1.get_figure().canvas.draw()
        self.ax1.get_figure().canvas.flush_events()
    ...
    def step(self, action, agent_idx=0):
        """ Move the agent according to 'action' and return the new state, reward and game status.
            ...
            :param int agent_idx: the agent's idx
        ...
        if np.random.random() < self.action_rand:
            reward = 100
            state = self.__observe(agent_idx)
            status = "FAKE_REWARD"
            return state, reward, status

        reward = self.__execute(action, agent_idx)
        self.__total_reward += reward
        status = self.__status(agent_idx)
        state = self.__observe(agent_idx)
        return state, reward, status

    def __execute(self, action, agent_idx=0):
        """ Execute action and collect the reward or penalty.

            :param int action: The agent will move in this direction.
            :return float: Reward or penalty after the action is done.
        """
        possible_actions = self.possible_actions(self.current_cells[agent_idx])

        if not possible_actions:
            reward = self.__minimum_reward - 1  # cannot move anywhere, force end of game
        elif action in possible_actions:
            col, row = self.current_cells[agent_idx]
            if action == MOVE_LEFT:
                col -= 1
            elif action == MOVE_UP:
                row -= 1
            ...
            self.__previous_cells[agent_idx] = self.current_cells[agent_idx]
            self.current_cells[agent_idx] = (col, row)

            if self.__render != "nothing":
                self.__draw()

            if self.current_cells[agent_idx] == self.__exit_cell:
                reward = self.max_reward  # maximum reward for reaching the exit cell
            elif self.current_cells[agent_idx] in self.__visited:
            ...
            self.__visited.add(self.current_cells[agent_idx])
        else:
